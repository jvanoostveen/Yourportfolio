...
<?php foreach ($GLOBALS['YP_LANGUAGES'] as $language_key => $language) : ?>
			...
			<textarea name="itemForm[item][metadata][seo_description][<?php echo $language_key; ?>]" id="seo_description_<?php echo $language_key; ?>" rows="4" cols="75"><?=$canvas->edit_filter($item->getMetadata('seo_description', $language_key));?></textarea><br>
			<label for="seo_keywords_<?php echo $language_key; ?>" class="floating_label"><?php echo _('keywords'); ?>:</label>
			<textarea name="itemForm[item][metadata][seo_keywords][<?php echo $language_key; ?>]" id="seo_keywords_<?php echo $language_key; ?>" rows="3" cols="75"><?=$canvas->edit_filter($item->getMetadata('seo_keywords', $language_key)); ?></textarea>
			</div>
<?php endforeach; ?>
		</div>
...
<?php else : ?>
		<div id="metadata_seo_fields">
			...
			<textarea name="itemForm[item][metadata][seo_description][default]" id="seo_description_default" rows="4" cols="75"><?=$canvas->edit_filter($item->getMetadata('seo_description', 'default'));?></textarea><br>
			<label for="seo_keywords_default" class="floating_label"><?php echo _('keywords'); ?>:</label>
			<textarea name="itemForm[item][metadata][seo_keywords][default]" id="seo_keywords_default" rows="3" cols="75"><?=$canvas->edit_filter($item->getMetadata('seo_keywords', 'default')); ?></textarea><br>
			</div>
		</div>
<?php endif; ?>
	</div>
...
<?php foreach ($GLOBALS['YP_LANGUAGES'] as $language_key => $language) : ?>
			...
			<input type="text" name="itemForm[item][metadata][og_title][<?php echo $language_key; ?>]" id="og_title_<?php echo $language_key; ?>" size="50" value="<?php echo $canvas->filter($item->getMetadata('og_title', $language_key)); ?>"><br>
			<label for="og_description_<?php echo $language_key; ?>" class="floating_label"><?php echo _('beschrijving'); ?>:</label>
			<textarea name="itemForm[item][metadata][og_description][<?php echo $language_key; ?>]" id="og_description_<?php echo $language_key; ?>" rows="4" cols="75"><?=$canvas->edit_filter($item->getMetadata('og_description', $language_key));?></textarea>
			</div>
<?php endforeach; ?>
		</div>
<script type="text/javascript">
...
<?php else : ?>
		<div id="metadata_og_fields">
			...
			<input type="text" name="itemForm[item][metadata][og_title][default]" id="og_title_default" size="50" value="<?php echo $canvas->filter($item->getMetadata('og_title', 'default')); ?>"><br>
			<label for="og_description_default" class="floating_label"><?php echo _('beschrijving'); ?>:</label>
			<textarea name="itemForm[item][metadata][og_description][default]" id="og_description_default" rows="4" cols="75"><?=$canvas->edit_filter($item->getMetadata('og_description', 'default'));?></textarea>
		</div>
<?php endif; ?>
	</div>
<script type="text/javascript">
